"""
wellfound.com Web Crawler

This crawler extracts data from wellfound.com.
Startup jobs and recruiting (formerly AngelList Talent)
...
class WellfoundCrawler:
    ...
    def parse_page(self, html: str, url: str) -> List[Dict[str, Any]]:
        ...
        # TODO: Customize parsing logic for wellfound.com
    ...
    def save_json(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            ...
            filename = f"wellfound_{timestamp}.json"
    ...
    def save_csv(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"wellfound_{timestamp}.csv"
...
async def main():
    ...
    async with WellfoundCrawler(output_dir="data/wellfound") as crawler:
        await crawler.crawl()
