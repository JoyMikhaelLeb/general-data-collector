...
This crawler extracts data from Harmonic.ai, a platform for startup and company data.
...
class HarmonicCrawler:
    """
    Crawler for Harmonic.ai platform.
    ...
    def parse_page(self, html: str, url: str) -> List[Dict[str, Any]]:
        ...
        # Example parsing logic - customize based on actual site structure
    ...
    def save_json(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            ...
            filename = f"harmonic_ai_{timestamp}.json"
    ...
    def save_csv(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"harmonic_ai_{timestamp}.csv"
...
async def main():
    ...
    async with HarmonicCrawler(output_dir="data/harmonic_ai") as crawler:
        await crawler.crawl()
