"""
data.sec.gov Web Crawler

This crawler extracts data from data.sec.gov.
SEC structured data API
...
class SecDataCrawler:
    """
    Crawler for data.sec.gov.
    ...
    def parse_page(self, html: str, url: str) -> List[Dict[str, Any]]:
        ...
        # TODO: Customize parsing logic for data.sec.gov
    ...
    def save_csv(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"sec_data_{timestamp}.csv"
...
async def main():
    ...
    async with SecDataCrawler(output_dir="data/sec_data") as crawler:
        await crawler.crawl()
