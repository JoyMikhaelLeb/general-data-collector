"""
www.sec.gov/edgar Web Crawler

This crawler extracts data from www.sec.gov/edgar.
...
class SecEdgarCrawler:
    ...
    def parse_page(self, html: str, url: str) -> List[Dict[str, Any]]:
        ...
        # TODO: Customize parsing logic for www.sec.gov/edgar
    ...
    def save_json(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            ...
            filename = f"sec_edgar_{timestamp}.json"
    ...
    def save_csv(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"sec_edgar_{timestamp}.csv"
...
async def main():
    ...
    async with SecEdgarCrawler(output_dir="data/sec_edgar") as crawler:
        await crawler.crawl()
