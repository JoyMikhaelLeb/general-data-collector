"""
growjo.com Web Crawler

This crawler extracts data from growjo.com.
Database of fastest-growing companies
...
class GrowjoCrawler:
    ...
    def parse_page(self, html: str, url: str) -> List[Dict[str, Any]]:
        ...
        # TODO: Customize parsing logic for growjo.com
    ...
    def save_json(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            ...
            filename = f"growjo_{timestamp}.json"
    ...
    def save_csv(self, filename: Optional[str] = None) -> Path:
        ...
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"growjo_{timestamp}.csv"
...
async def main():
    ...
    async with GrowjoCrawler(output_dir="data/growjo") as crawler:
        await crawler.crawl()
